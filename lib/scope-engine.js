...
export function createScopeEngine(options) {
  ...
  const alwaysVisibleNames = options.alwaysVisibleNames ?? (() => [])
  ...
  async function syncSession(record, reason) {
    ...
    // Exempted names (the companion guide) stay visible under every group.
    for (const name of alwaysVisibleNames()) visible.add(name)
