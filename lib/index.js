...
export function apply(ctx) {
  ...
  async function syncActiveGroup(forceGroupId) {
    if (syncing) { syncQueued = true; return } syncing = true; try {
      // Effective active group: an explicit override (from a session pick) wins,
      // otherwise fall back to the default group. Because the on-disk SKILL.md
      // names are global, applying an override re-shapes the shared catalog for
      // every session — this is the documented limitation of the disk-rename scheme.
      const activeId = forceGroupId !== undefined ? forceGroupId : (state.defaultGroup ?? null)
      let enabled = null
      if (activeId === ALL_OFF) { enabled = new Set() }
      else if (activeId) { const group = groupById(activeId); if (group) enabled = enabledNames(group) }
      const all = await scanSkills(); const failures = []; for (const skill of all) { try { const shouldEnable = enabled === null || enabled.has(skill.name); if (shouldEnable) await enableSkill(skill); else await disableSkill(skill) } catch (e) { failures.push(skill.name + ': ' + ((e && e.message) || e)) } } const known = {}; for (const s of all) known[s.dirName] = true; for (const k of Object.keys(state.disabled)) if (!known[k]) delete state.disabled[k]; await withConfigLock(async () => { await persist() }); if (failures.length) console.error('[dsh-skill-manager] sync partial failures', failures) } finally { syncing = false; if (syncQueued) { syncQueued = false; await syncActiveGroup(forceGroupId) } }
  }
  ...
  ctx.connection.rpc.handle('/skillmg', async (endpoint, payload) => {
    ...
    switch (endpoint) {
      ...
      case 'set-session-group': return await serial(async () => {
        ...
        // catalog so the current session's injection changes right away.
        await syncActiveGroup(gid)
