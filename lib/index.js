...
const GUIDE_SKILL_MD = [
  ...
  '- `skillmg_set_session_group` / `skillmg_get_session`：记录 / 查询当前会话的分组覆盖；`set_session_group` 会立即重排磁盘目录。',
  ...
  '- 最近一次生效的分组（无论来自会话选择还是默认组）会被保留；导入/删除/改分组等普通管理操作不会把它冲回默认。',
...
export function apply(ctx) {
  ...
  // ---- state & readiness ----
  let state = { sourceDirs: [], importTarget: defaultImportTarget, groups: [], defaultGroup: null, perSessionGroups: {}, disabled: {}, lastActive: null }
  ...
  ctx.connection.rpc.handle('/skillmg', async (endpoint, payload) => {
    ...
    switch (endpoint) {
      ...
      case 'set-session-group': return await serial(async () => {
        ...
        return { ok: true, activeGroup: gid, defaultGroup: state.defaultGroup }
      })
      case 'get-session': {
