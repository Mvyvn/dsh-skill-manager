...
import { createScopeEngine } from './scope-engine.js'

export const name = 'dsh-skill-manager'
export const inject = ['connection', 'tools', 'skills']
...
const GUIDE_MARKER = '<!-- skill-manager-guide: v2 -->'
...
const GUIDE_SKILL_MD = [
  ...
  '- 会话覆盖（perSessionGroups）：按会话 id 记录的单会话分组；设置后立即把磁盘目录重排到该分组（全局生效，别的会话也会跟着切换）。',
  ...
  '- `skillmg_set_session_group` / `skillmg_get_session`：设置 / 查询当前会话的分组；新版引擎下只影响这一个会话，下一步生效，磁盘不变。',
  ...
  '## 五、注意',
  '',
  '- 修改分组成员、默认组或某会话的分组后，下一次对话步骤立即生效；默认组为空 = 全部启用。',
  '- 每个会话的分组互相独立：A 会话选「科研」、B 会话选「嵌入式」可以同时成立，互不影响，也不会改动磁盘上的技能文件。',
  '- 极少数环境下（DSH 未提供按会话分层能力）插件会自动退回旧机制：通过改名 SKILL.md 控制可见性，此时切组会影响所有会话。`skillmg_get_config` 的 engine 字段会显示当前使用的引擎。',
  '- 本技能自身也在来源目录中，并且被插件豁免于分组停用；它始终可见。',
].join('\n')
...
export function apply(ctx) {
  ...
  // This initializer MUST NOT call joinPath()/sep(): sep() reads dshHome, which is
  // still in its temporal dead zone right here, so the fallback branch threw
  // "ReferenceError: Cannot access 'dshHome' before initialization". That branch is
  // the normal path when the host process has no DSH_HOME and the deployment does
  // not provide the dshHomePath service (0.1.7 web profile does not) — note that
  // tool subprocesses DO get DSH_HOME injected by dsh-shell-env, which is why this
  // only reproduced in a process the user started, never in a tool shell.
  const homePath = String(homedir()).replace(/[\\/]+$/, '')
  const dshHomeSource = (typeof dshHomeRaw === 'string' && dshHomeRaw.length > 0)
    ? 'dshHomePath'
    : (process.env.DSH_HOME ? 'DSH_HOME' : 'homedir')
  const dshHome = dshHomeSource === 'dshHomePath'
    ? dshHomeRaw
    : (dshHomeSource === 'DSH_HOME' ? process.env.DSH_HOME : homePath + (/\\/.test(homePath) ? '\\' : '/') + '.dsh')
  console.log('[dsh-skill-manager] dshHome=' + dshHome + ' (via ' + dshHomeSource + ')')
  ...
  // ---- state & readiness ----
  let state = { sourceDirs: [], importTarget: defaultImportTarget, groups: [], defaultGroup: null, perSessionGroups: {}, disabled: {}, lastActive: null, engine: 'auto', scopeNormalized: false }
  ...
  async function loadConfig() {
    ...
    if (cfg && typeof cfg === 'object') {
      ...
      state.lastActive = (typeof cfg.lastActive === 'string' && cfg.lastActive) ? cfg.lastActive : null
      state.engine = (cfg.engine === 'scope' || cfg.engine === 'rename') ? cfg.engine : 'auto'
      state.scopeNormalized = cfg.scopeNormalized === true
    } else {
      state.sourceDirs = []
    ...
    await cleanupLegacyMarkers()
    await ensureGuideSkill()
  }
  // Every config mutation bumps the revision; live sessions compare it on their
  // next step and re-evaluate only when something actually changed.
  let configRevision = 0
  function bumpRevision() { configRevision += 1; if (scopeEngine) scopeEngine.markAllDirty() }
  async function persist() { await writeJson(CONFIG_PATH, state); bumpRevision() }
  ...
  // One-time startup cleanup of the legacy `disable-model-invocation` marker.
  // The rename scheme (SKILL.md <-> SKILL.md.disable) never needs the marker to
  // exist (DSH only recognizes the exact filename `SKILL.md`), so instead of
  // reading-and-rewriting every file on every enable we wipe any leftover marker
  // line from a pre-rename install exactly once at startup.
  async function cleanupLegacyMarkers() {
    try {
      ...
      for (const skill of all) {
        for (const p of candidateMdPaths(skill)) {
          for (const f of [p.md, p.disabled]) {
            if (!(await nativeExists(f))) continue
            try {
              const text = await readText(f)
              if (text.indexOf(DISABLE_MARKER) >= 0) {
                const next = text.split('\n').filter((l) => l.trim() !== DISABLE_MARKER).join('\n')
                if (next !== text) { await writeText(f, next); cleaned += 1 }
              }
            } catch (e) { /* best effort */ }
          }
        }
      }
      if (cleaned) console.log('[dsh-skill-manager] legacy marker cleanup: cleaned ' + cleaned + ' file(s)')
    } catch (e) { console.error('[dsh-skill-manager] cleanupLegacyMarkers failed: ' + ((e && e.message) || e)) }
  }
  ...
  // ---- group sync ----
  async function syncActiveGroup(forceGroupId) {
    if (syncing) { syncQueued = true; return } syncing = true; try {
      // Effective active group: an explicit "current" (a session pick, or the
      // last group the user chose) wins, otherwise fall back to the default.
      // Because the on-disk SKILL.md names are global, applying a group re-shapes
      // the shared catalog for every session — this is the documented limitation
      // of the disk-rename scheme. state.lastActive remembers the most recent
      // on-disk group so ordinary management operations (import / delete / edit
      // group) keep the shape the user last chose instead of silently reverting
      // to the default and wiping a per-session override.
      // With a verified per-session engine, visibility lives in each session's own
      // registry layer: the disk must hold every imported skill as discoverable and
      // group changes never rewrite it again. syncActiveGroup therefore only
      // normalises the disk once and publishes a new revision.
      if (scopeMode()) {
        // Disk work is a one-time migration now, but the mutation that led here must
        // still be persisted: session-group picks (make it per-session) and
        // delete-skills (removes the skill from every group) mutate `state` and relied
        // on this call to write the file. Without this, those changes lived only in
        // memory and silently vanished on the next restart.
        await normalizeDiskForScope()
        await withConfigLock(async () => { await persist() })
        return
      }
      const activeId = forceGroupId !== undefined ? forceGroupId : (state.lastActive != null ? state.lastActive : (state.defaultGroup ?? null))
      let enabled = null
      if (activeId === ALL_OFF) { enabled = new Set() }
      else if (activeId) { const group = groupById(activeId); if (group) enabled = enabledNames(group) }
      const all = await scanSkills(); const failures = []; for (const skill of all) { try { // The companion guide is exempt from group policy: with a single-root preset (skill-only)
        // it is the only documentation of these tools the model can see, so hiding it would make
        // the AI unable to manage skills on its own.
        const shouldEnable = skill.dirName === GUIDE_NAME ? true : (enabled === null || enabled.has(skill.name)); if (shouldEnable) await enableSkill(skill); else await disableSkill(skill) } catch (e) { failures.push(skill.name + ': ' + ((e && e.message) || e)) } } const known = {}; for (const s of all) known[s.dirName] = true; for (const k of Object.keys(state.disabled)) if (!known[k]) delete state.disabled[k]; delete state.disabled[GUIDE_NAME]; state.lastActive = activeId; await withConfigLock(async () => { await persist() }); if (failures.length) console.error('[dsh-skill-manager] sync partial failures', failures) } finally { syncing = false; if (syncQueued) { syncQueued = false; await syncActiveGroup(forceGroupId) } }
  }
  ...
  async function ensureGuideSkill() {
    try {
      if (!state.sourceDirs.length) return
      const targets = [state.importTarget, state.sourceDirs[0]]
        .filter((d, i, a) => d && a.findIndex((x) => String(x).toLowerCase() === String(d).toLowerCase()) === i)
      for (const dir of targets) {
        const md = joinPath(joinPath(dir, GUIDE_NAME), 'SKILL.md')
        if (await existsAt(md)) {
          // Refresh a guide written by an older plugin version (new semantics must
          // reach the model without the user deleting the file by hand).
          try { if ((await readText(md)).indexOf(GUIDE_MARKER) >= 0) continue } catch (e) { continue }
        }
  ...
  // ---- per-session visibility engine (DSH >= 0.1.7) --------------------------
  // DSH's skill registry is layered per scope and the NEAREST layer wins a name,
  // so a session's own layer can shadow whatever its preset discovered while a
  // host-plane (global-layer) registration can never do so. Registering a
  // same-named entry with modelInvokable:false through agent.ctx hides a skill for
  // that session alone, in memory, with no disk writes and no preset edits.
  // Capability is proven on the first agent; any failure falls back to the
  // on-disk rename engine below (state.engine / scopeNormalized).
  let scopeEngine = null
  function scopeMode() {
    if (!scopeEngine) return false
    if (state.engine === 'rename') return false
    if (state.engine === 'scope') return true
    return scopeEngine.available === true || (scopeEngine.available === null && scopeEngine.optimistic())
  }
  // One-time migration: with in-memory visibility the disk must keep every
  // imported skill discoverable (the engine captures its real definition from the
  // provider in order to shadow it), so renamed .disable files come back.
  async function normalizeDiskForScope() {
    if (state.scopeNormalized) return
    try {
      let restored = 0
      for (const skill of await scanSkills()) {
        for (const p of candidateMdPaths(skill)) {
          if (await nativeExists(p.disabled)) { await renameFile(p.disabled, p.md); restored += 1 }
        }
      }
      state.disabled = {}
      state.scopeNormalized = true
      await withConfigLock(async () => { await persist() })
      if (restored) console.log('[dsh-skill-manager] scope engine: restored ' + restored + ' renamed skill file(s); visibility is per session now')
    } catch (e) { console.error('[dsh-skill-manager] normalizeDiskForScope failed: ' + ((e && e.message) || e)) }
  }
  scopeEngine = createScopeEngine({
    ctx,
    getState: () => state,
    getRevision: () => configRevision,
    scanManagedSkills: () => scanSkills(),
    // The companion guide must stay visible under every group, including all-off:
    // with a single-root preset it is the only documentation of these tools the
    // model can see (the rename engine exempts it the same way).
    alwaysVisibleNames: () => [GUIDE_NAME],
    log: (m) => console.log('[dsh-skill-manager] ' + m),
    warn: (m) => console.warn('[dsh-skill-manager] ' + m),
  })
  scopeEngine.start()
  console.log('[dsh-skill-manager] engine=' + (scopeMode() ? 'scope (per-session, in-memory)' : 'rename (on-disk)') +
    ', config=' + (state.engine || 'auto'))
  // ---- tool output helpers ----
  // DSH >= 0.1.7 validates every tool body against its declared output schema
  // (defineTool -> valueSchemaSpecToJsonSchema, then the registry's post-policy
  // check), so declaring `{ type: 'string' }` for tools that return structured
  // payloads fails with: tool "..." returned invalid output: "value" must be a
  // string. `json` is the author-facing node for "any JSON value" (the same
  // declaration @deepseek-ai/dsh-tool-cordis uses); the renderer below still
  // hands the model one JSON text block.
  const TOOL_OUTPUT_SCHEMA = { type: 'json' }
  function textOf(v) { return [{ type: 'text', text: JSON.stringify(v === undefined ? null : v) }] }
  function renderText() { return (_args, value) => textOf(value) }

  // ---- Connection RPC surface (the client calls it for every panel action) -----
  // DSH >= 0.1.7 mounts ONE browser channel (/api). A custom
  // `rpc.handle('/skillmg', …)` channel registers itself but produces NO reachable
  // route in that build — verified live: POST /skillmg/get-config answers 405 while
  // /api answers, which left the settings page stuck on "正在加载技能列表…" and its
  // group list empty even though the config was intact. The shared channel's single
  // interceptor slot is already owned by dsh-api-gateway, so the supported surface is
  // an exact Fetch route under /api (the same convention as /api/file and the upload
  // route). The client therefore calls POST /api/skillmg with { method, payload }.
  const skillmgRpc = async (endpoint, payload) => {
    ...
    switch (endpoint) {
      case 'get-config':
        return {
          ...
          engine: scopeMode() ? 'scope' : 'rename',
          rpc: rpcMode,
          // Under the scope engine nothing is disabled on disk; the number shown is
          // how many managed skills the default group hides for a new session.
          disabledCount: scopeMode() ? (scopeEngine ? (scopeEngine.hiddenCountFor(state.defaultGroup) ?? 0) : 0) : Object.keys(state.disabled).length,
          scopeEngine: scopeEngine ? scopeEngine.info() : null,
        }
      case 'scan': {
        try {
          const all = await scanSkills()
      ...
      case 'delete-group': return await serial(async () => {
        ...
        if (state.lastActive === id) state.lastActive = state.defaultGroup
      ...
      case 'set-default-group': return await serial(async () => {
        ...
        // An explicit default-group selection also becomes the current on-disk
        // shape, so later management ops re-sync to it (not to a stale override).
        state.lastActive = v
        await afterGroupChange()
      ...
      case 'set-session-group': return await serial(async () => {
        ...
        // "跟随默认" clears the override: fall back to the effective default
        // group (NOT enable-all). Passing undefined here would derive from
        // state.lastActive or defaultGroup; passing the defaultGroup explicitly
        // keeps semantics obvious and bug 1 (null -> enable-all) fixed.
        const effective = gid == null ? (state.defaultGroup ?? null) : gid
        if (gid == null) delete state.perSessionGroups[String(sid)]
        else state.perSessionGroups[String(sid)] = gid
        // Apply the picked group immediately (方案 A): re-shape the on-disk
        // catalog so the current session's injection changes right away. lastActive
        // is updated inside syncActiveGroup to keep ordinary management ops from
        // reverting the disk back to a stale default.
        await syncActiveGroup(effective)
        return { ok: true, activeGroup: gid, defaultGroup: state.defaultGroup, engine: scopeMode() ? 'scope' : 'rename' }
      })
      case 'get-session': {
        ...
        const effective = override != null ? override : state.defaultGroup
        return {
          sessionId: String(sid),
          activeGroup,
          fromOverride: override != null,
          engine: scopeMode() ? 'scope' : 'rename',
          hiddenCount: scopeMode() && scopeEngine ? (scopeEngine.hiddenCountFor(effective) ?? null) : null,
        }
      }
  ...
  let rpcMode = 'none'
  try {
    const fetchRegistry = ctx.connection && ctx.connection.fetch
    if (fetchRegistry && typeof fetchRegistry.register === 'function') {
      ctx.effect(() => fetchRegistry.register({
        path: '/api/skillmg',
        methods: ['POST'],
        ...
        fetch: async (request) => {
          ...
          try {
            const result = await skillmgRpc(body && body.method, body && body.payload)
            return Response.json({ result })
          } catch (e) {
            return Response.json({ error: String((e && e.message) || e) }, { status: 500 })
          }
        },
      }), 'dsh-skill-manager: /api/skillmg route')
      rpcMode = 'api-fetch-route'
    }
  } catch (e) { console.error('[dsh-skill-manager] /api/skillmg route registration failed: ' + ((e && e.message) || e)) }
  if (rpcMode === 'none' && ctx.connection && ctx.connection.rpc && typeof ctx.connection.rpc.handle === 'function') {
    // Older DSH: the per-channel registry is the only surface there.
    try {
      ctx.connection.rpc.handle('/skillmg', skillmgRpc, { authority: 'loopback' })
      rpcMode = 'legacy-channel'
    } catch (e) { console.error('[dsh-skill-manager] legacy rpc channel registration failed: ' + ((e && e.message) || e)) }
  }
  console.log('[dsh-skill-manager] rpc=' + rpcMode)
  ...
  register(defineTool({
    ...
    description: 'Record a per-session skill-group override by session id (or clear it with an empty group_id) and immediately re-shape the on-disk catalog to that group, so skills outside it are hidden. Clearing falls back to the default group.',
    parameters: {
  ...
  register(defineTool({
    ...
    execute: async (_args, exec) => {
      ...
      if (override != null) {
        return { sessionId: String(sessionId), activeGroup: override, fromOverride: true, engine: scopeMode() ? 'scope' : 'rename' }
      }
      const g = state.defaultGroup ? groupById(state.defaultGroup) : null
      const eff = override != null ? override : state.defaultGroup
      return {
        sessionId: String(sessionId),
        activeGroup: g ? g.id : null,
        fromOverride: false,
        engine: scopeMode() ? 'scope' : 'rename',
        hiddenCount: scopeMode() && scopeEngine ? (scopeEngine.hiddenCountFor(eff) ?? null) : null,
      }
    },
  }))
  ...
  register(defineTool({
    ...
    output: { schema: TOOL_OUTPUT_SCHEMA, render: renderText() },
    execute: async (_args, exec) => {
      const out = { snapshot: [], disabled: Object.keys(state.disabled), engine: scopeMode() ? 'scope' : 'rename', scopeEngine: scopeEngine ? scopeEngine.info() : null }
