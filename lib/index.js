...
export function apply(ctx) {
  ...
  async function syncActiveGroup(forceGroupId) {
    if (syncing) { syncQueued = true; return } syncing = true; try {
      ...
      const all = await scanSkills(); const failures = []; for (const skill of all) { try { const shouldEnable = enabled === null || enabled.has(skill.name); if (shouldEnable) await enableSkill(skill); else await disableSkill(skill) } catch (e) { failures.push(skill.name + ': ' + ((e && e.message) || e)) } } const known = {}; for (const s of all) known[s.dirName] = true; for (const k of Object.keys(state.disabled)) if (!known[k]) delete state.disabled[k]; state.lastActive = activeId; await withConfigLock(async () => { await persist() }); if (failures.length) console.error('[dsh-skill-manager] sync partial failures', failures) } finally { syncing = false; if (syncQueued) { syncQueued = false; await syncActiveGroup(forceGroupId) } }
  }
