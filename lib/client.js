  ...
  factory: (require) => {
    ...
    function parseZip(buf) {
      ...
      for (var j = 0; j < entries.length; j++) {
        var en = entries[j]
        if (!en.name || en.name.endsWith('/')) continue
        ...
        if (en.method === 0) jobs.push(Promise.resolve({ name: en.name, bytes: comp }))
        else if (en.method === 8) jobs.push(inflateRaw(comp).then(function (raw) { return { name: en.name, bytes: raw } }))
      }
