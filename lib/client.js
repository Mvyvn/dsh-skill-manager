...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // DSH >= 0.1.7 exposes plugin RPC only through the shared /api channel, so the
    // host registers one exact route at /api/skillmg taking { method, payload }.
    // The legacy per-channel envelope stays as a fallback for older DSH.
    function rpcCall(channel, endpoint, payload) {
      var request = { method: endpoint, payload: payload === undefined ? {} : payload }
      return fetch(new URL('/api/skillmg', window.location.origin), {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify(request),
      }).then(function (res) {
        if (res.status === 404 || res.status === 405) return legacyRpcCall(channel, endpoint, payload)
    ...
    function legacyRpcCall(channel, endpoint, payload) {
    ...
    // ---- skill-aware upload grouping ----
    // A skill is a directory that directly contains a SKILL.md. Uploaded files
    // are assigned to the deepest such directory they belong to; files not under
    // any skill directory are dropped, and the whole upload is rejected when no
    // SKILL.md exists at all.
    function _baseOf(p) { return p.indexOf('/') < 0 ? p : p.slice(p.lastIndexOf('/') + 1) }
    function _dirOf(p) { var i = p.lastIndexOf('/'); return i < 0 ? '' : p.slice(0, i) }
    function _depthOf(d) { return d === '' ? 0 : d.split('/').length }
    function _subOf(d, p) { return d === '' ? true : (p === d || p.indexOf(d + '/') === 0) }
    function _frontmatterName(text) {
      var m = /^---\s*\r?\n([\s\S]*?)\r?\n---/.exec(text || '')
      if (!m) return ''
      var lines = m[1].split(/\r?\n/)
      for (var i = 0; i < lines.length; i++) {
        var mm = /^\s*name\s*:\s*(?:"([^"]*)"|'([^']*)'|(\S+))/.exec(lines[i])
        if (mm) return (mm[1] || mm[2] || mm[3] || '').trim()
      }
      return ''
    }
    // Regroup raw upload paths into one-or-more detected skills.
    // Returns { ok:true, skills:[...], files:[{name,targetRel,content}] } or
    // { ok:false, reason } when no SKILL.md is found.
    function groupSkillUploads(files) {
      var markers = files.filter(function (f) { return _baseOf(f.rel) === 'SKILL.md' })
      if (!markers.length) return { ok: false, reason: '所选文件夹/压缩包中未扫描到任何 SKILL.md，未导入' }
      var dirs = []
      markers.forEach(function (m) { var d = _dirOf(m.rel); if (dirs.indexOf(d) < 0) dirs.push(d) })
      var rootMd = null
      markers.forEach(function (m) { if (_dirOf(m.rel) === '') rootMd = m })
      var rootName = rootMd ? _frontmatterName(rootMd.content) : ''
      var out = []
      var skills = []
      files.forEach(function (f) {
        var pick = null
        for (var i = 0; i < dirs.length; i++) {
          if (!_subOf(dirs[i], f.rel)) continue
          if (pick === null || _depthOf(dirs[i]) > _depthOf(pick)) pick = dirs[i]
        }
        if (pick === null) return
        var name
        if (pick === '') {
        ...
        // Always drop everything above the skill's own directory when writing
        // the destination path. webkitRelativePath is relative to whatever folder
        // the user happened to select, so it may include ancestor levels (e.g.
        // "skills/ui-ux-pro-max/SKILL.md" when a parent of the skill folder was
        // selected). A skill must always land at <importTarget>/<skillName>/...,
        // never nested under an ancestor directory.
        var targetRel = name + '/' + (pick === '' ? f.rel : f.rel.slice(pick.length + 1))
        if (skills.indexOf(name) < 0) skills.push(name)
        out.push({ name: name, targetRel: targetRel, content: f.content })
      })
      if (!out.length) return { ok: false, reason: '未解析到可导入的技能（根 SKILL.md 缺少 name 字段，或没有属于任何技能的文件）' }
      return { ok: true, skills: skills, files: out }
    }
    ...
    function ImportTab(props) {
      ...
      var deletingPair = useState(false)
      var deleting = deletingPair[0]
      var setDeleting = deletingPair[1]
      var delReportPair = useState(null)
      var delReport = delReportPair[0]
      var setDelReport = delReportPair[1]
      ...
      function doDelete() {
        if (!selNames.length || deleting) return
        setDeleting(true); setDelReport(null)
        rpcCall('/skillmg', 'delete', { names: selNames }).then(function (r) {
          setDelReport((r && r.reports) || []); setDeleting(false)
          // deselect deleted names and refresh the page
          setSelected({}); scan.reload(); cfg.reload()
        }).catch(function (e) { setDelReport([{ name: '操作失败', ok: false, reason: String((e && e.message) || e) }]); setDeleting(false) })
      }
      function doUpload(files, isZip) {
        ...
        var send = function () {
          ...
          rpcCall('/skillmg', 'upload', { files: payloads, overwrite: false }).then(function (res) {
            ...
            var n = groupedInfo ? groupedInfo.skills.length : 0
            finish({ ok: !!(res && res.ok), text: '已写入 ' + (res ? res.count : 0) + ' 个文件，共 ' + n + ' 个技能' + (existing ? '，' + existing + ' 个已存在已跳过' : '') + (rejected ? '，' + rejected + ' 个被拒绝' : '') + (failed ? '，' + failed + ' 个失败' : '') })
        ...
        var collectFile = function (rel, buf) {
          ...
          if (d.ok) collected.push({ rel: rel, content: d.text })
        ...
        Promise.all(jobs).then(function () {
          if (!collected.length) { finish({ ok: false, text: '没有可导入的文本文件（隐藏/系统文件已自动跳过）' }); return }
          var g = groupSkillUploads(collected)
          if (!g.ok) { finish({ ok: false, text: g.reason }); return }
          groupedInfo = g
          var seen = {}
          payloads = []
          for (var j = 0; j < g.files.length; j++) {
            var item = g.files[j]
            if (seen[item.targetRel]) continue
            seen[item.targetRel] = true
            payloads.push({ path: item.targetRel, content: item.content })
          }
          if (!payloads.length) { finish({ ok: false, text: '未解析到可导入的技能文件' }); return }
          send()
        }).catch(function (e) { finish({ ok: false, text: '上传失败：' + String((e && e.message) || e) }) })
      }
      ...
      var delOkCount = (delReport || []).filter(function (r) { return r.ok }).length
      return createElement('div', { className: 'skmg-box' },
        ...
        createElement('div', { className: 'skmg-hbar', style: { padding: '4px 0' } },
          createElement(Btn, { className: 'primary', disabled: busy || deleting || !selNames.length, onClick: doImport }, busy ? '导入中…' : '导入所选（' + selNames.length + '）'),
          createElement(Btn, { className: 'danger', disabled: busy || deleting || !selNames.length, onClick: doDelete }, deleting ? '删除中…' : '删除所选（' + selNames.length + '）')
        ),
        ...
        delReport ? createElement('div', {},
          createElement('div', { className: delOkCount === delReport.length ? 'skmg-ok' : 'skmg-error' }, '删除结果：成功 ' + delOkCount + ' / ' + delReport.length),
          delReport.filter(function (r) { return !r.ok }).map(function (r) { return createElement('div', { key: r.name + (r.reason || ''), className: 'skmg-error' }, '· ' + r.name + '：' + (r.reason || '失败')) })
        ) : null
      )
    }
    ...
    function GroupsTab(props) {
      ...
      return createElement('div', { className: 'skmg-box' },
        ...
        createElement('div', { className: 'skmg-hbar' },
          createElement('input', { className: 'skmg-input', placeholder: '新分组名称（如：我的分组）', value: newName, onChange: function (e) { setNewName(e.target.value) }, onKeyDown: function (e) { if (e.key === 'Enter') add() } }),
    ...
    function InputGroupPicker(props) {
      ...
      function load() {
        ...
        rpcCall('/skillmg', 'get-session', { sessionId: String(sessionId) }).then(function (d) { if (d) setCur(d.activeGroup || '') })
      }
